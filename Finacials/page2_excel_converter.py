#!/usr/bin/env python3
"""
Convert page2.csv (JUN - QUARTERLY WRITE OFFS) to professionally formatted Excel file
"""
...
def create_page2_excel():
    # Read the corrected CSV file
    csv_path = 'page2_corrected.csv'
    ...
    financial_cols = ['Last_Receipt_Payment', 'Current', '30_Days', '60_Days', '90_Days', '120_Days', '150_Days', 'Outstanding']
    ...
    exec_ws['A1'] = 'PROMED MEDICAL - QUARTERLY WRITE OFFS REPORT - PAGE 2'
    ...
    total_receipts = data_df['Last_Receipt_Payment'].sum()
    ...
    metrics = [
        ...
        ('Total Receipt Payments', f'R {total_receipts:,.2f}'),
    ...
    # Top Accounts
    row += 1
    exec_ws[f'A{row}'] = 'TOP 10 OUTSTANDING ACCOUNTS'
    exec_ws[f'A{row}'].font = subheader_font
    row += 1
    
    exec_ws[f'A{row}'] = 'Account ID'
    exec_ws[f'B{row}'] = 'Account Name'
    exec_ws[f'C{row}'] = 'Outstanding'
    exec_ws[f'A{row}'].font = header_font
    exec_ws[f'B{row}'].font = header_font
    exec_ws[f'C{row}'].font = header_font
    exec_ws[f'A{row}'].fill = header_fill
    ...
    top_10 = data_df.nlargest(10, 'Outstanding')[['Account_ID', 'Account_Name', 'Outstanding']]
    for idx, account_row in top_10.iterrows():
        exec_ws[f'A{row}'] = account_row['Account_ID']
        exec_ws[f'B{row}'] = account_row['Account_Name']
        exec_ws[f'C{row}'] = f"R {account_row['Outstanding']:,.2f}"
        exec_ws[f'A{row}'].font = normal_font
        exec_ws[f'B{row}'].font = normal_font
        exec_ws[f'C{row}'].font = normal_font
        row += 1
    ...
    exec_ws.column_dimensions['B'].width = 25
    exec_ws.column_dimensions['C'].width = 15
    
    # Create detailed data sheet
    ws = wb.create_sheet(title="JUN - Quarterly Write Offs P2")
    
    # Add title
    ws['A1'] = 'JUN - QUARTERLY WRITE OFFS - PAGE 2 - DETAILED DATA'
    ...
    # Save the file
    output_file = 'page2_corrected.xlsx'
    wb.save(output_file)
    ...
    print(f"Total Receipt Payments: R {total_receipts:,.2f}")
